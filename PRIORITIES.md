# PRIORITIES — generated from `state/projects.json` at 2026-09-08T00:59
...
4. **FitRival** — 75%, ETA ~8 agent-days
...
5. **openline** — 55%, ETA ~12 agent-days
...
5. **personhood** — 60%
   ...
   - needs user: DEPLOY the issuer (Fly.io, free tier): `brew install flyctl && fly auth login` (or `fly auth signup`), then from the repo root: `fly launch --no-deplo; SENDGRID (free, 100/day): create an API key with Mail Send at https://app.sendgrid.com/settings/api_keys and verify a Single Sender at https://app.sen; DEPLOY the web app (Vercel, free): `npm i -g vercel && vercel login`, then `cd app/web && vercel link && vercel env add NEXT_PUBLIC_PERSONHOOD_SERVER_
...
| ai-ubi-wellbeing-transition-simulator | 85 | yes | UBI transition simulator (conference demo) |
...
| oralhistory_timeline | 80 | partial (Play internal) | Oral history to interactive shareable timeline |
| sagearbor.github.io | 100 | yes | User site root: FitRival landing + privacy policy |
| taskcaster-app | 78 | web | Party game app; web MVP live, mobile stores not wired |
